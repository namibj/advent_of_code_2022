...
let part1_12 [n] (file: [n]u8) =
	let line_count = map ((== newline) >-> i32.bool) file |> reduce (+) 0 |> i64.i32
	let line_length = assert (n % line_count == 0) (n / line_count)
	let S = u8.i32 'S'
	let E = u8.i32 'E'
	let raw_grid = unflatten line_count line_length file
	let mapped_raw_grid = map (map (\x -> if x == S then u8.i32 'a' else if x == E then u8.i32 'z' else x)) raw_grid
	let valid_grid = raw_grid[:,:(line_length - 1)]
	let [n] [m] valid_mapped_grid: [n][m]u8 = mapped_raw_grid[:,:(line_length - 1)]
	let coord_zipped = map2 (\line i -> map2 (\x j -> (x, i, j)) line (indices line)) valid_grid (indices valid_grid) |> flatten
	let positions = spread 2 (-1, -1) (map (\(x, i, j) -> if x == S then 0 else if x == E then 1 else -1) coord_zipped) (map (\(x, i, j) -> (i, j)) coord_zipped)
	let (start_i, start_j) = positions[0]
	let (end_i, end_j) = positions[1]
	let direct_only = map2 (\line i -> map2 (\x j ->
			tabulate_2d n m (\k l -> 
				if i == k && j == l then 0 else
				if (i == k + 1 || i == k - 1) && j == l then
					if (valid_mapped_grid[i,j] + 1) >= valid_mapped_grid[k,l] then 1 else i16.highest else
				if i == k && (j == l + 1 || j == l - 1) then
					if (valid_mapped_grid[i,j] + 1) >= valid_mapped_grid[k,l] then 1 else i16.highest else
				i16.highest
			)
		) line (indices line)) valid_mapped_grid (indices valid_mapped_grid)
	let o = m * n
	let saturating_add_i16 (a) (b) =
		let res = (i32.i16 a) + (i32.i16 b)
		in if res > (i32.i16 i16.highest) then i16.highest else i16.i32 res
	let dotprod [n] (xs: [n]i16) (ys: [n]i16): i16 =
		(reduce (i16.min) (i16.highest) (map2 (saturating_add_i16) xs ys))
	let matmul [n][p][m] (xss: [n][p]i16) (yss: [p][m]i16): [n][m]i16 =
		map (\xs -> map (dotprod xs) (transpose yss)) xss
	let direct_only_1d = flatten_to o direct_only |> map (flatten_to o)
	let steps_needed = i64.num_bits - (i64.clz o) + 1 -- theoretically rounds up by one step in excess for power-of-two input sizes, but ignore that inefficiency
	let steps_needed = steps_needed + 1 -- bugfix attempt
	let finished_map = loop partial = direct_only_1d for x < steps_needed do
